...
while read FUNC; do
  ...
  sed -i '1d' $benchmark_name"-INPUT-"$gn"-"$FUNC".txt" # removes 1st string
...
do
  ...
  $LLVM10_buildMyPasses/bin/opt -load $pgsp_pass_path/libLLVMSWP.so -S -SWPrefetchingLLVMPass -input-file $benchmark_name"-INPUT-"$gn"-dist"$dist"-prefetch.afdo" -dist $dist <$benchmark_name".ll"> $benchmark_name"-pref-INPUT-"$gn"-dist"$dist".ll"
...
$LLVM10_buildMyPasses/bin/opt -load $pgsp_pass_path/libLLVMSWP.so -S -SWPrefetchingLLVMPass -input-file $benchmark_name"-INPUT-"$gn"-prefetch.afdo" -dist $dist  <$benchmark_name".ll"> $benchmark_name"-pref-INPUT-"$gn".ll"
